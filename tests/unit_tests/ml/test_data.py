...
def test_train_validation_length(resources):
    ...
    # Test origin data length with train and validation data length
    assert expected_data_length > len(train_images_absolute_paths) + len(
        validation_image_absolute_path
    )

    #  Test whether the size of the train dataset is larger than the validation dataset.

    assert len(train_images_absolute_paths) > len(validation_image_absolute_path)
