...
class SimplifiedMBConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, expand_ratio=1, dropout_rate=0.2):
...
class StudentEfficientNetB4(nn.Module):
    def __init__(self, num_classes=6, dropout_rate=0.2):
        ...
        self.blocks = nn.Sequential(
            SimplifiedMBConvBlock(32, 16, expand_ratio=1),
            SimplifiedMBConvBlock(16, 24, expand_ratio=6, stride=2, dropout_rate=dropout_rate),
            SimplifiedMBConvBlock(24, 40, expand_ratio=6, stride=2),
            SimplifiedMBConvBlock(40, 80, expand_ratio=6, stride=2, ),
            SimplifiedMBConvBlock(80, 112, expand_ratio=6, stride=1, ),
            SimplifiedMBConvBlock(112, 192, expand_ratio=6, stride=2),
            SimplifiedMBConvBlock(192, 320, expand_ratio=6, stride=1),
            SimplifiedMBConvBlock(320, 512, expand_ratio=6, stride=1),
            SimplifiedMBConvBlock(512, 720, expand_ratio=6, stride=1),
            SimplifiedMBConvBlock(720, 912, expand_ratio=6, stride=1),
            SimplifiedMBConvBlock(912, 1020, expand_ratio=6, stride=1, dropout_rate=dropout_rate)
        )
