...
class ModifiedStudentRegEfficientNetB4(nn.Module):
    def __init__(self, num_classes=6, dropout_rate=0.5):
        super(ModifiedStudentRegEfficientNetB4, self).__init__()
        ...
        self.blocks = nn.Sequential(
            ...
            SimplifiedMBConvBlock(112, 320, expand_ratio=6, stride=1, dropout_rate=dropout_rate),
            SimplifiedMBConvBlock(320, 512, expand_ratio=6, stride=2, dropout_rate=dropout_rate) 
        )
        
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        
        self.regressor = nn.Sequential(
            nn.Conv2d(512, 1020, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(1020, 1792, kernel_size=1)
        )
    ...
    def forward(self, x):
        ...
        conv_feature_map = self.blocks(x)
        regressor_output = self.regressor(conv_feature_map)
        x = self.global_pool(regressor_output)
        x = torch.flatten(x, 1)
        logits = self.fc(x)
